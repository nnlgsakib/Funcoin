...
FUNCOIN_CONFIG_ALL="--enable-suppress-external-warnings --disable-dependency-tracking --prefix=$DEPENDS_DIR/$HOST"
if [ -z "$NO_WERROR" ]; then
  FUNCOIN_CONFIG_ALL="${FUNCOIN_CONFIG_ALL} --enable-werror"
fi
...
if [ -n "$ANDROID_TOOLS_URL" ]; then
  ...
  CI_EXEC ./autogen.sh
  CI_EXEC ./configure "$FUNCOIN_CONFIG_ALL" "$FUNCOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)
...
FUNCOIN_CONFIG_ALL="${FUNCOIN_CONFIG_ALL} --enable-external-signer --bindir=$BASE_OUTDIR/bin --libdir=$BASE_OUTDIR/lib"
...
CI_EXEC "${BASE_ROOT_DIR}/configure" --cache-file=config.cache "$FUNCOIN_CONFIG_ALL" "$FUNCOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)

CI_EXEC make distdir VERSION="$HOST"

export P_CI_DIR="${BASE_BUILD_DIR}/bitcoin-$HOST"

CI_EXEC ./configure --cache-file=../config.cache "$FUNCOIN_CONFIG_ALL" "$FUNCOIN_CONFIG" || ( (CI_EXEC cat config.log) && false)
