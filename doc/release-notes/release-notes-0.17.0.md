Funcoin Core version 0.17.0 is now available from:
...
BIP 174 Partially Signed Funcoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Funcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Funcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Funcoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in FUN. This new field deprecates previous fee fields, such as
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in FUN (mryandao)
...
- #13528 `b877c39` Move FuncoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use FuncoinTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use FuncoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use FuncoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use FuncoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for FuncoinTestFramework (WillAyd)
