...
Funcoin Core version 0.20.0 is now available from:
...
- Instead of running `make` simply, you should instead run
  `FUNCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Funcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Funcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Funcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Funcoin Core. Funcoin Core logs debug messages
...
- OpenSSL is no longer used by Funcoin Core.  (#17265)

- BIP70 support has been fully removed from Funcoin Core. The
...
- All custom settings configured when Funcoin Core starts are now
...
- The "Start Funcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Funcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Funcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of Funcoin Core should support a GUI option
...
- Funcoin Core's automatically-generated source code documentation is
...
- #17910 Remove double `LIBFUNCOIN_SERVER` linking (fanquake)
...
- #18429 Remove double `LIBFUNCOIN_SERVER` from bench-Makefile (brakmic)
...
- #16983 Add detailed info about Funcoin Core files (hebasto)
...
- #18341 Replace remaining literal FUN with `CURRENCY_UNIT` (domob1812)
