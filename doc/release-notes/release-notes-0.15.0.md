...
Funcoin Core 0.15.0 contains the following changes to the RPC interface and `bitcoin-cli` for multi-wallet:

* When running Funcoin Core with a single wallet, there are **no** changes to the RPC interface or `bitcoin-cli`. All RPC calls and `bitcoin-cli` commands continue to work as before.
...
Funcoin Core has supported creating opt-in replace-by-fee (RBF) transactions
...
- The second argument in `prioritisetransaction` has been renamed from `priority_delta` to `dummy` since Funcoin Core no longer has a concept of coin age priority. The `dummy` argument has no functional effect, but is retained for positional argument compatibility. See [Removal of Coin Age Priority](#removal-of-coin-age-priority).
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a FUN/KB feerate (instagibbs)
...
- #10359 `329fc1d` [tests] functional tests should call FuncoinTestFramework start/stop node methods (jnewbery)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into FuncoinTestFramework (jnewbery)
...
- #10958 `659c096` Update to latest Funcoin patches for LevelDB (sipa)
