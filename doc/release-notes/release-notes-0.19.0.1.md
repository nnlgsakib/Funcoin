Funcoin Core version 0.19.0.1 is now available from:
...
Upgrading directly from a version of Funcoin Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of Funcoin Core are generally supported.

Compatibility
==============

Funcoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Funcoin Core on unsupported systems.
...
10.10. Additionally, Funcoin Core does not yet change appearance when
macOS "dark mode" is activated.

Users running macOS Catalina may need to "right-click" and then choose "Open"
to open the Funcoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Funcoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/bitcoin/bitcoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Funcoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Funcoin
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Funcoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Funcoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Funcoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Funcoin Core,
  ...
  This prevents Funcoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Funcoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Funcoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Funcoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Funcoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Funcoin Core will be bound to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Funcoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  FUN), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in FUN per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Funcoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Funcoin AEAD (jonasschnelli)
...
- #16521 Use the default maxfeerate value as FUN/kB (Remagpie)
...
- #15773 Add FuncoinTestFramework::sync_* methods (MarcoFalke)
...
- #15896 feature_filelock, interface_bitcoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Funcoin Core (luke-jr)
...
- #16929 follow-up to rpc: default maxfeerate value as FUN/kB (jonatack)
...
- #15683 Comment for seemingly duplicate LIBFUNCOIN_SERVER (Bushstar)
