# User-space, Statically Defined Tracing (USDT) for Funcoin Core

Funcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Funcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Funcoin Core
...
To list probes in Funcoin Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Funcoin Core.
