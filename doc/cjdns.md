# CJDNS support in Funcoin Core

It is possible to run Funcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Funcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Funcoin network and individual nodes.
...
before it will work with your Funcoin Core node. This is described in steps
...
## Run Funcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Funcoin Core
...
When enabled, this option tells Funcoin Core that it is running in an
...
IPv6 local network. This helps Funcoin Core perform better address management:
...
CJDNS support was added to Funcoin Core in version 23.0 and there may be fewer
...
## CJDNS-related information in Funcoin Core

There are several ways to see your CJDNS address in Funcoin Core:
