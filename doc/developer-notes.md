...
- [Development guidelines](#development-guidelines)
    - [General Funcoin Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Funcoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Funcoin Core is a multi-threaded application, and deadlocks or other
...
Funcoin Core can be compiled with various "sanitizers" enabled, which add
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `FUNCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef FUNCOIN_FOO_BAR_H
#define FUNCOIN_FOO_BAR_H
...
#endif // FUNCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of Funcoin Core, in which case
...
a Funcoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with Funcoin's `select()` loop, because
...
compatibility between nodes. This happened in Funcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Funcoin's
