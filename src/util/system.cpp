...
const char * const FUNCOIN_CONF_FILENAME = "bitcoin.conf";
const char * const FUNCOIN_SETTINGS_FILENAME = "settings.json";
...
{
    fs::path settings = GetPathArg("-settings", FUNCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Funcoin
    // macOS: ~/Library/Application Support/Funcoin
    // Unix-like: ~/.bitcoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const fs::path conf_path = GetPathArg("-conf", FUNCOIN_CONF_FILENAME);
