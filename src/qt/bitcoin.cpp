...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<FuncoinUnit>("FuncoinUnit");
#else
    qRegisterMetaType<FuncoinUnit>("FuncoinUnit");
#endif
}
...
FuncoinApplication::FuncoinApplication():
...
void FuncoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the FuncoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", FuncoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void FuncoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool FuncoinApplication::createOptionsModel(bool resetSettings)
{
...
void FuncoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new FuncoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &FuncoinGUI::quitRequested, this, &FuncoinApplication::requestShutdown);
...
void FuncoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
    ...
    m_splash->show();
    connect(this, &FuncoinApplication::splashFinished, m_splash, &SplashScreen::finish);
    connect(this, &FuncoinApplication::requestedShutdown, m_splash, &QWidget::close);
}

void FuncoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool FuncoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void FuncoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &FuncoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &FuncoinApplication::handleRunawayException);
    connect(this, &FuncoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &FuncoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void FuncoinApplication::parameterSetup()
{
...
void FuncoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void FuncoinApplication::requestInitialize()
{
...
void FuncoinApplication::requestShutdown()
{
...
void FuncoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &FuncoinGUI::handlePaymentRequest);
            connect(window, &FuncoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void FuncoinApplication::handleRunawayException(const QString &message)
{
...
void FuncoinApplication::handleNonFatalException(const QString& message)
{
...
WId FuncoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool FuncoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FuncoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
