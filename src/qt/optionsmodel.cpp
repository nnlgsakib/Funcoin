...
/** Call node.updateRwSetting() with Funcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Funcoin 22.x releases try to read these specific settings as
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayFuncoinUnit")) {
        settings.setValue("DisplayFuncoinUnit", QVariant::fromValue(FuncoinUnit::FUN));
    }
    QVariant unit = settings.value("DisplayFuncoinUnit");
    if (unit.canConvert<FuncoinUnit>()) {
        m_display_bitcoin_unit = unit.value<FuncoinUnit>();
    } else {
        m_display_bitcoin_unit = FuncoinUnit::FUN;
        settings.setValue("DisplayFuncoinUnit", QVariant::fromValue(m_display_bitcoin_unit));
    }
...
{
    if (new_unit.isNull() || new_unit.value<FuncoinUnit>() == m_display_bitcoin_unit) return;
    m_display_bitcoin_unit = new_unit.value<FuncoinUnit>();
    QSettings settings;
    settings.setValue("DisplayFuncoinUnit", QVariant::fromValue(m_display_bitcoin_unit));
