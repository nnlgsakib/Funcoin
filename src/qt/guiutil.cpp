...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new FuncoinAddressEntryValidator(parent));
    widget->setCheckValidator(new FuncoinAddressCheckValidator(parent));
}
...
bool parseFuncoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!FuncoinUnits::parse(FuncoinUnit::FUN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseFuncoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseFuncoinURI(uriInstance, out);
}

QString formatFuncoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FuncoinUnits::format(FuncoinUnit::FUN, info.amount, false, FuncoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openFuncoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetPathArg("-conf", FUNCOIN_CONF_FILENAME));
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Funcoin (%s).lnk", chain));
}
