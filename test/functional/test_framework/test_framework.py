#!/usr/bin/env python3
# Copyright (c) 2014-2021 The Funcoin Core developers
...
class FuncoinTestMetaClass(type):
    """Metaclass for FuncoinTestFramework.

    Ensures that any attempt to register a subclass of `FuncoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'FuncoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("FuncoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("FuncoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class FuncoinTestFramework(metaclass=FuncoinTestMetaClass):
    ...
    def setup(self):
        ...
        self.options.bitcoind = os.getenv("FUNCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("FUNCOINCLI", default=fname_bitcoincli)
        self.options.bitcoinutil = os.getenv("FUNCOINUTIL", default=fname_bitcoinutil)
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("FuncoinRPC")
    ...
    def is_bitcoin_util_compiled(self):
        ...
        return self.config["components"].getboolean("ENABLE_FUNCOIN_UTIL")
