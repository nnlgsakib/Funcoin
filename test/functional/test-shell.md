...
The `TestShell` submodule extends the `FuncoinTestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `FuncoinTestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `FuncoinTestFramework` between
...
## 2. Importing `TestShell` from the Funcoin Core repository

We can import the `TestShell` by adding the path of the Funcoin Core
...
The `TestShell` inherits all `FuncoinTestFramework` members and methods, such
as:
...
`FuncoinTestFramework` object. The full set of argument keywords which can be
...
Unlike the `FuncoinTestFramework` class, the `TestShell` keeps the underlying
Funcoind subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `bitcoind` node
processes and `FuncoinTestFramework` convenience methods can be accessed
interactively.
...
underlying `FuncoinTestFramework`:
...
`FuncoinTestFramework` class. The user can override these in
